...
// see FPD_Transport
// (button) means: 0 for release, 1 for switch (if release is not supported), 2
// for hold (if release should be expected) (hold) means: 0 for release, 2 for
// hold (jog) means: value is an integer increment if FPT_Jog, FPT_StripJog,
// FPT_MarkerJumpJog, FPT_MarkerSelJog, FPT_Previous or FPT_Next don't answer,
// FPT_PreviousNext will be tried. So it's best to implement at least
// FPT_PreviousNext if FPT_PunchIn or FPT_PunchOut don't answer, FPT_Punch will
// be tried if FPT_UndoUp doesn't answer, FPT_UndoJog will be tried if
// FPT_AddAltMarker doesn't answer, FPT_AddMarker will be tried if FPT_Cut,
// FPT_Copy, FPT_Paste, FPT_Insert, FPT_Delete, FPT_NextWindow, FPT_Enter,
// FPT_Escape, FPT_Yes, FPT_No, FPT_Fx don't answer, standard keystrokes will be
// simulated
const int FPT_Jog = 0; // (jog) generic jog (can be used to select stuff)
const int FPT_Jog2 =
    1; // (jog) alternate generic jog (can be used to relocate stuff)
const int FPT_Strip = 2;    // touch-sensitive jog strip, value will be in
                            // -65536..65536 for leftmost..rightmost
const int FPT_StripJog = 3; // (jog) touch-sensitive jog in jog mode
const int FPT_StripHold =
    4; // value will be 0 for release, 1,2 for 1,2 fingers centered mode, -1,-2
...
const int FPT_UndoUp =
    21; // (button) undo up in history (no need to implement if no undo history)
const int FPT_UndoJog =
    22; // (jog) undo in history (no need to implement if no undo history)
